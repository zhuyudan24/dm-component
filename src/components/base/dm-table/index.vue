<template>
  <el-table class="egrid" :data="data" ref="grid" v-bind="tableBind" v-on="$listeners">
    <!-- <template v-for="tp in typesColumns">
      ...
      <el-table-column v-else :key="tp.type" :type="tp.type" v-bind="tp.props"> </el-table-column>
    </template>
    <el-table-column v-for="col in renderColumns" :key="col.label" v-bind="getColBind(col)">
      <template slot-scope="scope">
        <component :is="col.component" v-bind="getCptBind(scope, col)" v-on="col.listeners"> </component>
      </template>
    </el-table-column>
...
// const BOOLEAN_KEYS = ['fit', 'stripe', 'border', 'show-header', 'highlight-current-row', 'default-expand-all', 'show-summary'];
// const COLUMN_PROPS = {
//   align: 'left',
//   component: Text
// };
...
export default {
  ...
  //   },
  //   renderColumns() {
  //     const { columns, columnKeyMap, columnsHandler, columnsProps: props, columnsSchema: schema } = this;
