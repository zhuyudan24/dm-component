<template>
  <section class="dm-store">
    ...
    <option-type v-if="storeType === 1" :uuid="newUuid" :scenesVal="scenesVal" :cacheUuid="cacheUuid" :isAdd="isAdd" :isCache.sync="isSendCache" @option-change="optionChange"></option-type>
    <option-tags v-if="storeType === 2" :uuid="newUuid" :scenesVal="scenesVal" :cacheUuid="cacheUuid" :isAdd="isAdd" :isCache.sync="isSendCache" @option-change="optionChange"></option-tags>
    <option-area v-if="storeType === 3" :uuid="newUuid" :scenesVal="scenesVal" :cacheUuid="cacheUuid" :isAdd="isAdd" :isCache.sync="isSendCache" @option-change="optionChange"></option-area>
    <option-group v-if="storeType === 4" :uuid="newUuid" :scenesVal="scenesVal" :cacheUuid="cacheUuid" :isAdd="isAdd" :isCache.sync="isSendCache" @option-change="optionChange"></option-group>
    <option-part v-if="storeType === 5" :uuid="newUuid" :scenesVal="scenesVal" :cacheUuid="cacheUuid" :isAdd="isAdd" :isCache.sync="isSendCache" @option-change="optionChange"></option-part>
  </section>
</template>
  ...
  methods: {
    ...
    getStoreConfig() {
      return new Promise((resolve, reject) => {
        ...
        this.axios
          .post(baseUrl + '/api-plug/get-store-widget-config?requestProject=gic-web', qs.stringify(params))
          .then(res => {
    ...
    saveInit(isCacheUuid = false) {
      ...
      this.axios
        .post(baseUrl + '/api-plug/save-store-widget?requestProject=gic-web', qs.stringify(params))
        .then(res => {})
        .catch(err => {
          log(err);
        });
    ...
    isStoreSave() {
      return new Promise((resolve, reject) => {
        ...
        this.axios
          .post(baseUrl + '/api-plug/is-empty?requestProject=gic-web', qs.stringify(params))
          .then(res => {
            if (res.data.errorCode === 0) {
