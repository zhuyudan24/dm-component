<template>
  <!-- 导出数据 -->
  <el-dialog title="导出数据" :visible.sync="myDialogVisible" width="500px" @close="closeDialog" :append-to-body="appendToBody">
    <div class="outer-dialogwrap">
      <ul class="clearfix">
        <li class="item " :class="{ fl: type == '2' }">
          <div class="img">
            <img src="./data1.svg" alt="脱敏" />
          </div>
    ...
    <el-dialog width="680px" title="下载申请" :visible.sync="dialogDndVisible" append-to-body>
...
export default {
  ...
  methods: {
    ...
    senReport() {
      ...
      if (this.isReadyReportBool) {
        this.isReadyReportBool = false;
        this.axios.post(this.excelUrl, qs.stringify(this.params)).then(res => {
          let data = res.data;
          if (data.errorCode == 0) {
            this.goReportBool = true;
            this.isReadyReportBool = true;
        ...
        this.$message.error('正在打包您要下载的数据，请稍候...');
      }
    },
    senGoReport() {
      // 脱敏数据 完整数据  去报告中心
