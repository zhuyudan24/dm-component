        ...
        <div class="filter-content" v-if="conditionsShow">
          ...
          <div class="filter-group marginLeft134">
            ...
            <div id="app1" class="rations">
              <!-- 循环几行(并且)-->
              <div class="condition-cell" v-for="(item, index) in conditionsList" :key="index">
                  ...
                  <div class="group_combo" data-group-id="1" v-for="(cellItem, key) in item.children" :key="key">
                    ...
                    <div class="filter-message groupbox">
                      ...
                      <template v-for="(childCell, ind) in cellItem.children">
                        <div class="gic-select-wrap inline-block" :key="ind" v-if="childCell.templateCode == 'com021' && childCell.storeFlag">
                          <dm-store-group :options="childCell.storeOption" :key="ind" :uuid.sync="childCell.uuid" :isAdd="childCell.operateType" ref="storeGroup"></dm-store-group>
                        </div>
...
import dmStoreGroup from '../dm-store-group/index.vue';
import FilterTxt from './filter-txt';
import tags from './tags';
import GicSelect from './birth-select';

export default {
  name: 'vue-gic-group-people',
  props: {
    flag: {
      type: Boolean,
      default: false
  ...
  methods: {
      ...
      this.axios
        ...
        .then(res => {
          ...
          if (resData.errorCode == 0) {
            ...
            // 门店
            if (data.property === 19 && data.widget.templateCode === 'com021') {
    ...
    addNextData(item, fieldKey, indexObj) {
      ...
      if (item.property === 19 && item.widget.templateCode === 'com021') {
    ...
    selectFocus(index, key, order) {
      ...
      // sy 回显数据
      this.axios.get(`${this.baseUrl}/api-plug/get-screening-widget-chain-detail?requestProject=gic-clique&widgetChainId=${idCollection.esScreeningWidgetChainId}`).then(res => {
    ...
    deleteCell(index, key) {
      // this.conditionsList.forEach(v => {
      //   if (v.children instanceof Array) {
      //     v.children.forEach(w => {
      //       alert(w.children[1].uuid);
      //     });
      //   }
      // });
    ...
    editTemp() {
      ...
      if (this.repId && !this.confirm) {
        ...
        this.$nextTick(_ => {
          this.editList = JSON.parse(JSON.stringify(this.conditionsList));
    ...
    async StoreHasEmpty() {
      ...
      const storeList = this.$refs.storeGroup;
  ...
  watch: {
    ...
    projectName(newData, oldData) {
      this.repProjectName = newData || 'gic-clique';
    },
  ...
  mounted() {
    this.getDataList();
    // 项目名
    this.repProjectName = this.projectName || 'gic-clique';
